...
|---|---|
| `LibForever-1.0.lua` | **Core.** Events and callbacks: `On(event, fn)`, `Listen`, `Fire`, `Debounce`. Identity: `Me`, `FullName`, `ShortName`, `ColorName`. The guild roster, from the updates the server sends: `roster`, `IsOnline`, `IsGuildie`. Guild comms that queue during chat lockdown: `RegisterComm`, `Send` (needs AceComm-3.0). Map distance in yards: `MyPosition`, `Distance`. Shared data: `ProvideData`, `GetData`. Saved-variable defaults and migrations: `PrepareDB(db, defaults, migrations, version)`. |
